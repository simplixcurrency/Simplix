...
Core tests take longer than any other Simplix tests, due to the high amount of computational work involved in validating core components.
...
To run only Simplix's core tests (after building):
...
To run only Simplix's crypto tests (after building):
...
```
simplixd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `simplix-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Simplix's hash tests (after building):
...
To run only Simplix's performance tests (after building):
...
To run only Simplix's unit tests (after building):
